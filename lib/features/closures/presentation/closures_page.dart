...
class _ClosuresPageState extends ConsumerState<ClosuresPage> {
  ...
  Future<void> _load() async {
    ...
    final unitId = session.user?.role?.isGlobal == true
        ? null
        : session.user?.unitId;
    ...
    setState(() {
      _loading = false;
      if (pending case AppSuccess(data: final d)) {
        _pending = d;
      } else if (pending case AppFailure(error: final e)) {
        _error = e.message;
      }
      if (closed case AppSuccess(data: final d)) {
        _closed = d;
      }
    });
  }
  ...
  Future<void> _closeOrder(FlightOrder order) async {
    ...
    final result = await ref
        .read(closuresRepositoryProvider)
        .closeOrder(order.id);
    if (!mounted) return;
    switch (result) {
      case AppSuccess():
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          TextButton.icon(
            onPressed: () => context.go('/flight-orders'),
            icon: const Icon(Icons.arrow_back, size: 18),
            label: const Text('Volver a Órdenes de Vuelo'),
          ),
          const SizedBox(height: 12),
          Expanded(
            child: Row(
...
class _OrderTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final dateStr =
        '${order.operationDate.day.toString().padLeft(2, '0')}/${order.operationDate.month.toString().padLeft(2, '0')}/${order.operationDate.year}';
    return ListTile(
      dense: true,
      title: Text(
        'OV $dateStr',
